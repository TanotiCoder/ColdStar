...
class PopularFilmSource(private val apiService: ApiService, private val filmType: FilmType) :
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Film> {
        return try {
            ...
            val treadingFilmData = if (filmType == FilmType.MOVIE)
                apiService.getPopularMovie(page = nextPage)
            else
                apiService.getPopularTvSeries(page = nextPage)
            LoadResult.Page(
