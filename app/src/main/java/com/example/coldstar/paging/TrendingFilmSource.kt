...
class TrendingFilmSource(private val apiService: ApiService, private val filmType: FilmType) :
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Film> {
        return try {
            ...
            val treadingFilmData = if (filmType == FilmType.MOVIE)
                apiService.getTrendingMovie(page = nextPage)
            else
                apiService.getTrendingTvSeries(page = nextPage)
            LoadResult.Page(
